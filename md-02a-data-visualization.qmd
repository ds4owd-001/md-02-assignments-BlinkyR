...
1.  Answer: the data types used for this map includes categorical variables (continent) and continuous variables (life expectancy)
...
ggplot(data = gapminder_2007,
  ...
  ggtitle("Boxplot with Hidden Outliers")
```
...
4.  ***The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:***
    ...
    Geom_col() is counting the frequency of the variable (continent) and ploting it which is similar to making a bar chart of categorical data using geom_bar.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    ***The 'break' argument sets the boundaries of a chart based on a specified range of values. Without it, the x-axis starts from 38 and extends slightly beyond 85. When using 'break', the chart begins at 40 and ends at 85. The seq() function defines respectively the minimum, maximum values for the x-axis, and width of each bin.***
